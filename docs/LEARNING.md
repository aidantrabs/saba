...
- You can save a plan to a file with `terraform plan -out=tfplan` and then run `terraform apply tfplan` to apply _exactly_ that plan with no confirmation prompt. This is the pattern used in CI/CD pipelines to ensure what was reviewed is what gets applied.

### 2. What is _state_ and why does Terraform need it?
...
### 3. What's a _provider_ and why do you need to configure one?
...
### 4. What's the difference between a _resource_ and a _data source_?
...
- A **private subnet** is a subnet associated with a route table that _doesn't_ have a route to an Internet Gateway. Resources in private subnets cannot communicate with the public internet.
...
![[Pasted image 20260113150744.png]] ![[Pasted image 20260113150816.png]]
...
- **Elastic IP + NAT Gateway** — allows private subnets to reach the internet _outbound only_
...
- **Why this is okay:** Public-facing resources _need_ to be reachable by users on the internet
...
> _A VPC with no compute is just an empty network._ You need to add:
> 
...
![[Pasted image 20260115131033.png]]
...
![[Pasted image 20260115143017.png]]
...
![[Pasted image 20260116084412.png]]
